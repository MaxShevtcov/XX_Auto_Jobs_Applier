...
**Revision:** v3 (зафиксировано распределение форум-топиков существующего чата `@xx_feedback`; добавлен опциональный секрет `tg_control_topic_id`; вопрос «какой чат использовать» закрыт)
...
Единый long-running Python-процесс объединяет: (1) Telegram-бота на python-telegram-bot с интерактивным меню — просмотр/изменение расписания автопоиска, ручной запуск `/search`, генерация писем `/letter`; (2) встроенный шедулер на **APScheduler** (`AsyncIOScheduler` + `CronTrigger`), который ежедневно запускает тот же `run_search_pipeline()`. Расписание персистится в `data_folder/schedule/schedule.yaml` (volume уже смонтирован). Ручной запуск и шедулер делят общий `asyncio.Lock`. Пропущенные запуски обрабатываются политикой APScheduler (`misfire_grace_time` + `coalesce`). Основной сценарий разового запуска `main.py` сохраняется для локальной отладки.
...
| `tg_jobs_topic_id` (345) | Без изменений: карточки вакансий + письма после успешных откликов из основного пайплайна (`send_job_description`). Дополнительно — дубликат результата `/letter` как «архив письма» (опционально, см. Open Questions) |
...
**A. Один процесс = бот + шедулер.** Новый корневой модуль `app_main.py` собирает: `PlaywrightJobManager` (лениво), `SearchScheduler`, `HhApplierBot`, запускает их в одном event loop. Шедулер и хендлеры бота вызывают один и тот же `run_search_pipeline()` через один `PipelineRunner`-обёртку с общим `asyncio.Lock`.
...
misfire_grace_time_sec: 3600   # догонять ли пропущенный запуск и как долго
```
...
**D. Часовой пояс.** Хранится в `schedule.yaml`, передаётся в `CronTrigger(timezone=...)`. Дефолт `Europe/Moscow` (аудит hh.ru). Валидация через `zoneinfo.ZoneInfo`.

**E. Защита от параллельного запуска.** Единый объект `SearchRunner` (см. Phase 3): `asyncio.Lock` + метод `run(force, progress_cb, source: "manual"|"scheduled")`. И хендлер `/search`, и job шедулера идут через него. Если занято:
- ручной запуск получает отказ «Поиск уже выполняется»;
- scheduled-запуск просто пропускается с warning'ом в лог (следующий по cron).

**F. Пропущенные запуски.** `misfire_grace_time_sec` из конфига (дефолт 3600): если контейнер был выключен в плановое время и стартовал в пределах grace — APScheduler выполнит один догоняющий запуск (`coalesce=True`). Позже grace → запуск пропущен до следующего cron. Плюс независимая страховка от слишком частых прогонов — `check_the_last_search_time()` в самом пайплайне.
...
```
`schedule:set` — FSM-lite на python-telegram-bot (`ConversationHandler`): бот ждёт сообщение вида `30 8 * * * Europe/Moscow` или упрощённое `09:30`, валидирует (cron-парсинг через `CronTrigger.from_crontab` + `ZoneInfo`), показывает превью следующих 3 запусков (`trigger.get_next_fire_time`), кнопки «Сохранить/Отмена».
...
### Phase 2: SearchRunner — общий исполнитель с lock'ом для шедулера и ручного запуска

**Objective:** Единая точка запуска поиска: `asyncio.Lock`, статус «занят/свободен», различие источника запуска, интеграция с прогрессом в Telegram.

**Files to Modify/Create:**
- `src/application/search_runner.py` (новый):
  ```python
  class SearchRunner:
      ...
      async def run(self, *, force=False, source="manual", progress_cb=None) -> dict | None:
          """None — если уже запущено."""
  ```
  Внутри: захват lock, `manager = await manager_factory()`, вызов `run_search_pipeline(..., manager=manager)`; в finally — release lock, сброс статуса.
- `data_folder/schedule/schedule.yaml` — схема описана в решении C (создаётся Phase 4, но константа пути `SCHEDULE_FILE` добавляется здесь в `src/constants.py`).

**Tests to Write:** `tests/test_search_runner.py`
- `test_run_rejects_when_lock_held` (второй вызов → None)
- `test_run_sets_and_resets_status` (is_running, started_at, source)
- `test_last_run_info_reads_cache_file` (tmp_path с last_run.yaml)
- `test_exception_does_not_leak_lock`

**Acceptance Criteria:**
- [ ] Любые два конкурентных запуска невозможны; статус доступен боту и шедулеру

---

### Phase 3: Скелет long-running приложения: шедулер (APScheduler) + каркас бота

**Objective:** Процесс, который живёт постоянно: `AsyncIOScheduler` читает расписание из `schedule.yaml` и триггерит `SearchRunner.run(source="scheduled")`; бот отвечает на `/start`, `/status`, `/menu` (меню пока с рабочими «Искать сейчас»/«Статус», остальные пункты-заглушки).

**Files to Modify/Create:**
- `requirements.txt`: добавить `APScheduler>=3.10,<4`
- `src/constants.py`: `SCHEDULE_FILE = "data_folder/schedule/schedule.yaml"`, `DEFAULT_SCHEDULE_CRON = "0 9 * * *"`, `DEFAULT_SCHEDULE_TZ = "Europe/Moscow"`
...
- `src/application/search_scheduler.py` (новый) — класс `SearchScheduler`:
  - `__init__(runner: SearchRunner, store: ScheduleStore)`
  ...
  - `_fire()`: если `runner.is_running` → logger.warning + пропуск; иначе `await runner.run(source="scheduled", progress_cb=self._log_progress)`; все исключения ловятся и логируются (job не должен валить процесс)
...
- `src/telegram/telegram_bot.py` (новый) — класс `HhApplierBot` (каркас из v1 + long-running):
  ...
  - `/start` (приветствие + меню), `/status` (свободен/занят + источник + время старта + последний прогон из `runner.last_run_info()` + следующее время по расписанию), `/menu` (inline-клавиатура) — все ответы через `router.control()`
...
- `app_main.py` (корень, новый) — точка входа long-running: валидация секретов/конфига → ленивая фабрика `PlaywrightJobManager` → `SearchRunner` → `ScheduleStore` → `SearchScheduler.start()` → `HhApplierBot(...)` + graceful shutdown (SIGTERM/SIGINT: `scheduler.shutdown()`, `await manager.close()`)
...
- `tests/test_search_scheduler.py`:
  ...
  - `test_fire_runs_scheduled_search` (мок runner)
  - `test_fire_skips_when_already_running`
...
- `tests/test_telegram_bot.py`:
  ...
  - `test_status_reports_idle_and_busy` (busy — при зажатом runner._lock)
...
### Phase 4: Ручной запуск /search и пункт меню «Искать сейчас» (из v1, адаптирована под SearchRunner)

**Objective:** `/search [force]` и callback `search:now` запускают `SearchRunner.run(source="manual")` с live-статусом и финальной сводкой.

**Files to Modify/Create:**
- `src/telegram/telegram_bot.py` — расширение:
  - Хендлер `/search`: занято → отказ; `force`-аргумент или кнопка подтверждения (callback `search:confirm:force` / `search:cancel`) при суточном лимите; иначе `asyncio.create_task(self._run_manual(progress_editor, force))`
  - `_run_manual`: `ProgressMessenger` (новый класс ниже) → `runner.run(force, source="manual", progress_cb=messenger)` → сводка (кол-во откликов, причина остановки); исключение → короткое сообщение пользователю, полный tb в лог
...
- `test_search_rejects_when_lock_held`
- `test_search_requires_confirmation_without_force`
- `test_search_runs_and_reports_result` (мок SearchRunner.run)
- `test_search_failure_sends_error_message`
- `test_progress_messenger_throttles_edits`

**Acceptance Criteria:**
- [ ] Повторный `/search` во время выполнения (в т.ч. запущенного шедулером) отклоняется
- [ ] Ошибка пайплайна не оставляет lock зажатым и не валит процесс
...
- Edge cases: шедулер выключен (`enabled=false`) — `/status` показывает «автозапуск отключён»; попытка `toggle` во время выполняющегося поиска — разрешена (влияет только на будущие запуски)
...
### Phase 6: Извлечение вакансии + /letter (из v1, валидна без изменений)

**Objective:** `/letter <ссылка|текст>` → письмо в чат через существующий механизм резюме и анонимизации.

**Files to Modify/Create:**
- `src/job_manager/vacancy_extractor.py` (новый): regex `hh\.ru/vacancy/(\d+)` → `manager.get_vacancy_full_info(url)` → `Job(**job).model_dump()`; без ссылки — `Job(job_title="Вакансия из чата", description=text)` (обрезка ~15000 символов)
...
- `src/telegram/telegram_bot.py` — хендлер `/letter`: пустой аргумент → подсказка; «⏳ Генерирую…» → письмо; ошибка → сообщение
- Конкурентность: операции с браузером идут через тот же механизм, что и поиск (общий ленивый `PlaywrightJobManager` приложения; `/letter` проверяет `runner.is_running` и отказывается во время поиска, либо использует отдельный `browser_semaphore` — но проще единый отказ, т.к. менеджер один)
...
- bot: пустые аргументы, отказ при занятом поиске, успешная отправка письма

**Acceptance Criteria:**
- [ ] Резюме — существующий механизм (скрейп + TTL), персональные данные анонимизируются до LLM
- [ ] Нет гонки `/letter` ↔ поиск за единственный браузер
...
- [ ] `docker stop` не оставляет битых состояний (lock/кэш)
...
- **Шедулер тестируется детерминированно**: не ждём реального времени — проверяем конфигурацию job'ов (`get_job().trigger`, `misfire_grace_time`, `coalesce`) и вызываем `_fire()` напрямую с моком `SearchRunner`; для проверки cron-парсинга — `CronTrigger.from_crontab(...)` + `get_next_fire_time` на фиксированном `now` (freezegun/подстановка datetime не требуется, API APScheduler принимает `now`)
...
| Риск | Митигация |
|---|---|
| APScheduler job падает и молча умирает | try/except вокруг `_fire` + logger.error; `max_instances=1` |
| Поиск длится дольше интервала до следующего cron-запуска | `SearchRunner.is_running` → scheduled-запуск пропускается с warning; `max_instances=1` в APScheduler как второй рубеж |
| Контейнер перезапущен во время поиска | Кэш `last_run.yaml`/`last_apply` уже восстанавливает счётчики (`_check_the_previous_apply_number`); misfire догоняет пропущенное расписание |
...
> Закрыт в v3: чат для бота — существующий `@xx_feedback` с форум-топиками; распределение зафиксировано в решении H.

1. **Дефолтное расписание из коробки**: `0 9 * * * Europe/Moscow` — подходит? Нужно ли брать TZ из search_config вместо secrets/файла?
2. **Misfire policy по умолчанию**: догонять ли пропущенный запуск в течение часа (`misfire_grace_time=3600, coalesce=True`) или строго «только в своё время» (grace=0)? Выбран первый вариант, подтверждение?
3. **Поведение scheduled-запуска при занятом ручном поиске**: пропуск до следующего cron (выбрано) или постановка в очередь/отложенный догон?
4. **`/letter` во время поиска**: жёсткий отказ (выбрано — браузер один) или очередь после завершения поиска?
5. **Авторизация**: достаточно ли привязки к `tg_chat_id` + опционального allowlist, или нужен полноценный multi-user (несколько клиентов, каждому своё расписание)? Multi-user существенно усложнит `schedule.yaml` (нужен профиль на пользователя).
6. **HH API vs Playwright** для вакансии по ссылке: рекомендован Playwright (консистентность, живая сессия) — подтверждение?
7. **Дублировать ли результат `/letter`** в `tg_jobs_topic_id` как «архив писем», или он остаётся только в управляющем топике? (Частично снимает прежний вопрос об аудит-логе: история команд и так видна в control-топике.)

## Success Criteria
- [ ] Все фазы завершены; `pre-commit run --all-files` проходит
- [ ] Контейнер long-running: `restart: unless-stopped`, внешний cron не нужен
- [ ] Шедулер внутри контейнера триггерит `run_search_pipeline` по cron из `schedule.yaml`; расписание переживает рестарты; misfire обрабатывается
- [ ] Все сообщения бота направляются в правильные топики `@xx_feedback`; интерактив (меню/статусы/прогресс) — в `tg_control_topic_id` (или fallback на report-топик)
- [ ] Меню бота: просмотр/изменение расписания, вкл/выкл автозапуска, ручной `/search` со статусом
- [ ] Общий lock исключает параллельные запуски (шедулер ↔ ручной)
- [ ] `/letter` работает через существующий механизм резюме + анонимизации
